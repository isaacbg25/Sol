...
do j= 0,365
    do i = 0,95 !perquè ens dona en el temps i+1
        ...
        k_1 = -(g*m_s/r(i,0)**2) + ((l**2)/(m_t**(2)*r(i,0)**3))
        !K_2 = (1/(r(i,0)+K_1*dt*0.5)**2) + (1/(r(i,0)+K_1*dt*0.5)**3)
        !k_3 = (1/(r(i,0)+K_2*dt*0.5)**2) + (1/(r(i,0)+K_2*dt*0.5)**3)
        !k_4 = (1/(r(i,0)+K_3*dt)**2) + (1/(r(i,0)+K_3*dt)**3)
        v(i+1,0)=v(i,0)+K_1*dt!(dt/6)*(k_1+2*K_2+2*k_3+k_4)
        c_1 = v(i,0)
        !c_2 =v(i)+0.5*dt*c_1
        !c_3 = v(i)+0.5*dt*c_2
        !c_4 = v(i)+dt*c_3
        r(i+1,0)= r(i,0)+dt*c_1 !(dt/6)*(c_1+2*c_2+2*c_3+c_4)
        r(i,j)=r(i,0)+j*r(i,0)
    end do
