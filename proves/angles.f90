...
real(8) :: r_0(n), phi, rho(n), r_s(n), r_phi(n,n), r_pla(n), r_betha(n,n), r_real(n), rho_pla(n)
real(8) ::  a_v(0:96,0:365), a_h(0:96,0:365), rho_0(n), r_betha_inv(n,n), r_gamma(n,n), r_gamma_inv(n,n)
...
do j= 0,365
    do i = 0,95 !perquè ens dona en el temps i+1
        ...
        !K_2 = (1/(r(i,j)+K_1*dt*j.5)**2) + (1/(r(i,j)+K_1*dt*j.5)**3)
        !k_3 = (1/(r(i,j)+K_2*dt*j.5)**2) + (1/(r(i,j)+K_2*dt*j.5)**3)
    ...
    theta(0,j+1)=theta(96,j)+dt*L/(m_t*r(96,j)**2)
    k_1 = -(g*m_s/r(96,j)**2) + ((l**2)/(m_t**(2)*r(96,j)**3))
    v(0,j+1)=v(96,j)+K_1*dt!(dt/6)*(k_1+2*K_2+2*k_3+k_4)
    c_1 = v(96,j)
    r(0,j+1)= r(96,j)+dt*c_1 !(dt/6)*(c_1+2*c_2+2*c_3+c_4)
end do
...
r_gamma = reshape([cos(gamma),sin(gamma), 0.0_8, -sin(gamma), cos(gamma),0.0_8,0.0_8,0.0_8,1.0_8], shape(r_gamma))
...
do j=0,365
    ...
    do i= 0,96
        ...
        r_real = matmul(r_pla,r_gamma)
        rho =r_s + r_real
        rho_pla = matmul(rho,r_gamma_inv)
        rho_0= matmul(rho_pla, r_betha_inv)
        a_h(i,j) =pi-acos((rho_0(1)*r_0(1)+rho_0(2)*r_0(2))/(sqrt(rho_0(1)**2+rho_0(2)**2)*sqrt(r_0(1)**2+r_0(2)**2)))
        a_v(i,j) = -pi/2 + acos((r_real(1)*rho(1)+r_real(2)*rho(2)+r_real(3)*rho(3))/(sqrt(sum(r_real**2))*sqrt(sum(rho**2))))
    end do
